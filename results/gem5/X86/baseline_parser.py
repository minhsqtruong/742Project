...
def get_file_addr():
    ...
    # coreid = input("Which core id do you want to parse? just type <Enter> if needed ")
...
def plot_stackbar(addr, apps, fields, pos, plottitle="Specify Title", ytitle="Specify Title", legend=None, H="/", glimpse=True, show=False, save_dir=None):
    '''
    addr: the address generated by get_file_addr() function
    apps: applications we are parsing
    fields: list of fields we want to stack
    pos: position of value within line
    plottitle: title of plot
    ytitle: title of y axis
    legend: instead of gem5 legend, use something reading friendly
    ...
    for app in apps:
        for field in fields:
            raw = get_stats(addr=addr, field=field, pos=pos, app=app, dataset=None)
            values = []
            for key in raw:
                if (key in datasets):
    ...
    dfall = []
    for key in bars:
        dfall.append(pd.DataFrame(bars[key], index = apps))
    ...
    if (legend is not None):
        l1 = axe.legend(h[:n_col], legend, loc=[1.01, 0.5])
    else:
...
if __name__ == '__main__':
    ...
    plot_stackbar(addr, apps = ["bfs", "pr", "tc"],
                  fields=["IntAlu",
                          "IntMult",
                          "IntDiv",
                          "No_OpClass"], pos=2,
                  plottitle="Operations breakdown", ytitle="Percentage (%)", show=True)
    plot_stackbar(addr, apps = ["bfs", "pr", "tc"], fields=["num_int_register_reads", "num_int_register_writes", "MemRead", "MemWrite"],
                  pos=1, plottitle="R/W accesses breakdown", ytitle="Access counts",
                  legend=["Register Reads", "Register Writes", "Mem Reads", "Mem Writes"], show=True)
    plot_stackbar(addr, apps = ["bfs", "pr", "tc"], fields=["dcache.demand_misses::total", "dcache.demand_hits::total"], pos=1,
                  plottitle="DCache hit/miss breakdown", ytitle="Counts",
                  legend=["Misses", "Hits"], show=True)
    plot_stackbar(addr, apps = ["bfs", "pr", "tc"], fields=["l2.demand_misses::total", "l2.demand_hits::total"], pos=1,
                  plottitle="L2 hit/miss breakdown", ytitle="Counts",
                  legend=["Misses", "Hits"], show=True)
